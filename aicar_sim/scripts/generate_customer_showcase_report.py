...
from aicar_sim.customer_showcase import build_customer_showcase_report  # noqa: E402
...
DEFAULT_OUTPUT_PATH = (
    PROJECT_ROOT / "outputs" / "customer_showcase" / "stage3_customer_showcase_report.html"
)
STAGE2_OUTPUTS = {
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Generate Stage3.3 customer showcase HTML.")
    parser.add_argument(
        "--vehicle-type-result",
    ...
    parser.add_argument(
        ...
        help="Output path for stage3_customer_showcase_report.html.",
    )
    return parser
...
def main() -> None:
    ...
    vehicle_type_result = load_json(vehicle_type_result_path)
    data = {name: load_json(path) for name, path in STAGE2_OUTPUTS.items()}
    html_text, summary = build_customer_showcase_report(
        vehicle_type_result,
