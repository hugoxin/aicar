...
DEFAULT_OUTPUT_PATH = PROJECT_ROOT / "outputs" / "path_plan" / "abstract_nozzle_path_plan.json"
VEHICLES_DIR = PROJECT_ROOT / "data" / "vehicles"


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Generate Stage2.5 abstract nozzle path plan.",
    )
    parser.add_argument(
        "--vehicle-type-result",
    ...
    parser.add_argument(
        "--wash-flow-run",
        default=str(DEFAULT_FLOW_RUN_PATH.relative_to(WORKSPACE_ROOT)),
        help="Path to wash_flow_run.json.",
    )
    parser.add_argument(
        "--space-model-report",
    ...
    parser.add_argument(
        ...
        help="Output path for abstract_nozzle_path_plan.json.",
    )
    return parser
...
def main() -> None:
    ...
    flow_run_path = resolve_workspace_path(args.wash_flow_run)
    ...
    write_json(output_path, path_plan)

    print(f"vehicle_type: {path_plan['vehicle']['vehicle_type']}")
    print(f"wash_profile: {path_plan['wash_profile']}")
    print(f"state_count: {path_plan['summary']['state_count']}")
    print(f"segment_count: {path_plan['summary']['segment_count']}")
    print(f"point_count: {path_plan['summary']['point_count']}")
    print(f"estimated_total_seconds: {path_plan['summary']['estimated_total_seconds']}")
