...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run the Stage3.2 timeline animation demo.")
    parser.add_argument(
        "--vehicle-type-result",
...
def run_generator(aicar_root: Path, vehicle_type_result: Path) -> Path:
    report_path = (
        ...
        / "outputs"
        / "timeline_animation"
        / "stage3_timeline_animation_report.html"
    )
    command = [
        sys.executable,
        str(aicar_root / "aicar_sim" / "scripts" / "generate_timeline_animation_report.py"),
        "--vehicle-type-result",
...
def build_summary(aicar_root: Path) -> dict:
    ...
    flow_run = load_json(aicar_root / STAGE2_JSON_OUTPUTS["wash_flow_run"])
    return {
        "vehicle_type": coverage["vehicle"]["vehicle_type"],
        "wash_profile": coverage["wash_profile"],
        "estimated_total_seconds": flow_run["summary"]["estimated_total_seconds"],
...
def main() -> None:
    ...
    demo_report = dirs["reports"] / "stage3_timeline_animation_report.html"
